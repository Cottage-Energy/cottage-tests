...
```
Billing users (isHandleBilling=true, maintainedFor IS NOT NULL):
  null (scraped) → approved (autoApproveBills) → processed (balance-ledger-batch) → done
                                                  ↘ cancelled

Non-billing users (isHandleBilling=false, maintainedFor IS NULL):
  viewable (inserted directly by scraper — never approved)

Wrongfully-approved edge case:
  Non-billing bill inserted/promoted to `approved` manually (PG-Admin or direct INSERT) →
    balance-ledger-batch skips it (checks maintainedFor) → stays approved forever
    No retroactive transition to `viewable` exists.
```

**Key insight**: `viewable` is NOT a terminal state of an `approved→viewable` transition. Non-billing bills are **born `viewable`** by `checkNewBills()` in `automations/src/base/utility.ts:309-315` (nightly audit scraper). The only way to get a non-billing bill in `approved` is manual intervention, and those get stuck because the downstream cron filters by `maintainedFor`.
...
| `isDelinquent` | boolean | Delinquency flag (set by reminder pipeline AND by `PaymentProcessor.recalculateDelinquency()` on every successful payment) |
...
**Transaction status lifecycle** (verified in live dev 2026-04-14 — 14642 APPLIED / 6163 INFLIGHT / 3859 QUEUED / 1342 VOID / 51 REJECTED):

```
                    skip_queue=true        skip_queue=true
                    inflight=false         inflight=true
                    ┌─────────┐           ┌──────────┐
  createTransaction │ APPLIED │           │ INFLIGHT │
  ────────────────►│ (final) │            │          │
                    └─────────┘           └────┬─────┘
                                               │ commit → APPLIED
                                               │ void   → VOID
                    ...
                    skip_queue=false
                    ┌────────┐   queue     ┌──────────┐
  createTransaction │ QUEUED │──worker────►│ INFLIGHT │
  ────────────────►│         │             │          │
                    └────────┘             └────┬─────┘
                                                │ commit → APPLIED
                                                │ void   → VOID
...
- **QUEUED**: initial state when `skip_queue=false`. BLNK's internal queue worker asynchronously picks up and transitions to `INFLIGHT` (if `inflight: true`) or directly `APPLIED`. Auto-pay is the primary `skip_queue=false` path (`services/packages/utilities/src/payments/payment-processor.ts:~130`). Bill ingestion (`packages/ledgers/services/bill-ingestion.ts:~130`) and manual pay use `skip_queue=true` — they never enter QUEUED.
- **REJECTED**: legacy / dormant — 51 observed, all pre-Nov 2025. No active code path creates this. Treat as read-only historical data.
- **Test-side handling**: `getFinalTransactionStatus()` in services repo treats `queued` like `inflight` (continues traversing children to find terminal status). `processPayments` in `packages/ledgers/services/balance.ts:~255` explicitly maps `QUEUED → INFLIGHT` for balance calculations. Our polling-based assertions in `tests/resources/fixtures/database/blnkQueries.ts` handle the transient state naturally, but `checkTransactionStatus` signature must accept `QUEUED` to avoid type errors on any future single-shot assertion.
...
- **Only billing users** (`maintainedFor IS NOT NULL`) have bills processed through the payment pipeline. Non-billing users' bills are inserted directly as `viewable` by the nightly audit scraper (`automations/src/base/utility.ts:309-315 checkNewBills()`) and never leave that state. If a non-billing bill ends up `approved` (via PG-Admin or manual INSERT — "wrongfully approved"), it will stay `approved` forever because `balance-ledger-batch` filters by `maintainedFor`.
...
Updated in **two places** using the same `calculateDelinquency()` logic and emitting the same `ledger.delinquency.tag` Inngest event:

1. **Reminder pipeline cron** (`ledger.payment.reminders`) — side effect during daily reminder processing.
2. **On every successful payment event** — via `PaymentProcessor.recalculateDelinquency()` (`services/packages/utilities/src/payments/payment-processor.ts:~233`, `~290`). Triggered by manual pay, auto-pay, and auto-pay-reconciliation paths. **NOT** triggered by the raw Stripe webhook handlers — the recalculation happens inside `processPayment()` before the webhook-path handlers run.

Trigger matrix:

| Payment path | Uses `PaymentProcessor` | Recalculates delinquency |
|--------------|-------------------------|--------------------------|
| Manual "Pay bill" (`packages/lambda/src/manual-payment/index.ts`) | Yes | Yes |
| Auto-pay (bill ingestion, `packages/inngest/functions/billing/ledger/apply/index.ts`) | Yes | Yes |
| Auto-pay reconciliation | Yes | Yes |
| Stripe webhook `payment_intent.succeeded` (separate `payment_success_process` fn) | No | No (remittances/emails only) |
| Stripe webhook `payment_intent.payment_failed` (separate `payment_failed_process` fn) | No | No (voids txn, disables auto-pay if non-recoverable) |
| Reminder cron | N/A (uses `calculateReminderLogic`) | Yes |
