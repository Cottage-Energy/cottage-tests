...
| PR-005 | Delinquency tagging — via reminder pipeline | Bill overdue, user NOT auto-paid | Trigger `ledger.payment.reminders` cron | `isDelinquent = true`, `delinquentDays = N` set on ElectricAccount/GasAccount by `ledger.delinquency.tag` event (emitted from reminder cron). Verify via DB poll. | P2 | @regression3 | No (Inngest) |
| PR-005a | Delinquency tagging — via payment success (manual pay) | Account has `isDelinquent = true`, user manually pays full outstanding balance | Click "Pay bill" → submit → wait for payment succeeded | `PaymentProcessor.recalculateDelinquency()` fires (services `packages/utilities/src/payments/payment-processor.ts:~233`). `isDelinquent` → `false`, `delinquentDays` → `0` WITHOUT waiting for reminder cron. Verify `updatedAt` on account happens within ~30s of payment. | P1 | @regression2 | ✅ [delinquency_on_payment_events.spec.ts](../e2e_tests/payment/delinquency_on_payment_events.spec.ts) |
| PR-005b | Delinquency tagging — via auto-pay success | Account `isDelinquent = true`, auto-pay ON, new bill ingested | Wait for `balance-ledger-batch` → payment success | `recalculateDelinquency()` clears delinquency on payment success (same code path as manual). Reminder cron does NOT need to run first. | P1 | @regression2 | ✅ [delinquency_on_payment_events.spec.ts](../e2e_tests/payment/delinquency_on_payment_events.spec.ts) |
| PR-005c | Delinquency tagging — partial payment | Account `isDelinquent = true`, 30 days overdue. User pays half via "Other Amount" option in manual pay modal. | Click "Pay bill" → select "Other Amount" radio → enter partial $ amount → submit | `recalculateDelinquency()` fires. `calculateDelinquency()` re-evaluates — if balance still >= $1.00 AND daysOverdue > 0: `isDelinquent` stays `true`, `delinquentDays` recomputed. UI verified live 2026-04-14: partial payment via "Other Amount" radio + `$` textbox is available in the standard Pay Bill modal. | P2 | @regression3 | ✅ [delinquency_on_payment_events.spec.ts](../e2e_tests/payment/delinquency_on_payment_events.spec.ts) |
| PR-005d | Delinquency NOT updated by Stripe webhook | Stripe `payment_intent.succeeded` fires but no PaymentProcessor call | Observe account after raw webhook | `isDelinquent` unchanged by `payment_success_process` Inngest fn alone. Delinquency recalc happens inside `PaymentProcessor.processPayment()` BEFORE the webhook path — webhook only handles remittances/emails. | P3 | @regression4 | No (infra-level) |
...
| DB-008 | Non-billing user — bill born `viewable` via scraper | Non-billing account (`maintainedFor IS NULL`, `isHandleBilling = false`), nightly-audit scraper runs with a new bill | Let `checkNewBills()` insert the bill | Bill inserted **directly as `ingestionState = "viewable"`** — never passes through `approved`. No BLNK transaction. No Payment. No `ChargeAccount` processing. `viewable` is a terminal state (old viewable bills prove no transition path exists). Code ref: `automations/src/base/utility.ts:309-315`. | P1 | @regression2 | ✅ [non_billing_bill_handling.spec.ts](../e2e_tests/payment/non_billing_bill_handling.spec.ts) (electric + gas + stasis) |
| DB-008a | Wrongfully-approved non-billing bill — stuck forever | Non-billing account, bill manually INSERT'd with `ingestionState = "approved"` (or promoted via PG-Admin) | Wait multiple `balance-ledger-batch` cron cycles | Bill **stays `approved` forever**. `balance-ledger-batch` filters `WHERE maintainedFor IS NOT NULL` — skips the bill silently. No BLNK txn, no Payment, no retroactive transition to `viewable`. This is Cian's edge case. Dev currently has 12 electric + 3 gas stuck bills. | P2 | @regression3 | ✅ [non_billing_bill_handling.spec.ts](../e2e_tests/payment/non_billing_bill_handling.spec.ts) (electric + gas variants) |
| DB-008b | Non-billing bill visibility on FE | Non-billing user with `viewable` bills | Sign in → navigate to Billing & Payments | Bills displayed in Bill History (read-only). No "Pay bill" button. No outstanding balance. | P2 | @regression3 | Yes |
...
### 20. BLNK Transaction State Lifecycle

| ID | Title | Preconditions | Steps | Expected Result | Priority | Tag | Automate? |
|----|-------|---------------|-------|-----------------|----------|-----|-----------|
| BLQ-001 | QUEUED — auto-pay payment transaction starts QUEUED | Billing user, auto-pay ON, card, approved bill entering ledger batch | Insert approved bill → wait for `balance-ledger-batch` → immediately poll `blnk.transactions` by reference `electric_{paymentID}` | First observable status is `QUEUED` (skip_queue=false path in `packages/utilities/src/payments/payment-processor.ts:~130`). Transitions to `INFLIGHT` after BLNK queue worker picks it up. | P1 | @regression2 | **Intentionally not automated** — QUEUED→INFLIGHT is sub-second; race window too tight for a reliable assertion. BLQ-006 (historical QUEUED counts) and BLQ-007 (balance treats QUEUED as INFLIGHT) already prove the lifecycle exists. Leave as documentation-only. |
| BLQ-002 | QUEUED → INFLIGHT transition | Auto-pay transaction just created | Poll `blnk.transactions` every 500ms for up to 30s | Observable sequence: `QUEUED` → `INFLIGHT`. Timing: usually < 2s. Documentation only — don't assert on exact timing. | P2 | @regression3 | **Not automated** — same race-window rationale as BLQ-001. Keep as doc-only guidance. |
| BLQ-003 | Bill ingestion txn NEVER in QUEUED | Approved bill being processed | Poll by reference `electric-bill-{billID}` from the moment `ingestionState` flips `approved → processed` | Status observed is `APPLIED` directly — bill ingestion uses `skip_queue: true` (`packages/ledgers/services/bill-ingestion.ts:~130`), bypasses queue. | P1 | @regression2 | ✅ [blnk_state_lifecycle.spec.ts](../e2e_tests/payment/blnk_state_lifecycle.spec.ts) |
| BLQ-004 | Manual pay txn NEVER in QUEUED | Manual pay submission | Scan Payments by users with `CottageUsers.isAutoPaymentEnabled=false` in last 7 days, assert no linked BLNK txn observed in QUEUED | First observable status is `INFLIGHT` — manual pay uses `skip_queue: true`. Transitions to `APPLIED` on success. **Discriminator**: `CottageUsers.isAutoPaymentEnabled=false` identifies manual-mode users (confirmed via Supabase 2026-04-14). | P1 | @regression2 | ✅ [blnk_state_lifecycle.spec.ts](../e2e_tests/payment/blnk_state_lifecycle.spec.ts) |
| BLQ-005 | `checkTransactionStatus` helper accepts QUEUED | n/a | Static type check of `tests/resources/fixtures/database/blnkQueries.ts:210` | Signature includes `'QUEUED'` as a valid expected status. Polling retry loop continues while status is `QUEUED` (treated as transient). | P1 | @regression1 | ✅ [blnk_state_lifecycle.spec.ts](../e2e_tests/payment/blnk_state_lifecycle.spec.ts) |
| BLQ-006 | REJECTED status — dormancy guard | Legacy data from pre-Nov 2025 | Query `blnk.transactions` over rolling 60-day window | 51 historical records exist; no active code path creates `REJECTED`. Rolling-window assertion catches any code-path regression that starts producing REJECTED. | P3 | @regression5 | ✅ [blnk_state_lifecycle.spec.ts](../e2e_tests/payment/blnk_state_lifecycle.spec.ts) |
| BLQ-007 | Balance calculation treats QUEUED as INFLIGHT | Auto-pay txn in QUEUED state | Query BLNK balance while txn is still QUEUED | `processPayments` in `packages/ledgers/services/balance.ts:~255` explicitly maps `QUEUED → INFLIGHT` for balance calculations. `inflight_debit_balance` reflects QUEUED amount the same as INFLIGHT. | P2 | @regression3 | ✅ [blnk_state_lifecycle.spec.ts](../e2e_tests/payment/blnk_state_lifecycle.spec.ts) (skips if no live QUEUED in 30d) |
