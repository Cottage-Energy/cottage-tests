...
### Session: 2026-04-17/18 (ENG-2188 password-reset retest — "email received" false-PASS)
- **"Email received" is NEVER a PASS on auth recovery / verification flows.** During ENG-2188 retests on Apr 14 + Apr 15, "Forgot password" was marked ✅ in both consolidated summaries — evidence was "submitted email, reset email received (10,482 chars)." The user asked me to retest end-to-end; a 30-second grep caught it: the link was never clicked, the form was never seen, the password was never changed, sign-in was never attempted. Filed as ENG-2721 (CRITICAL — TanStack `/auth/confirm` 500s on valid recovery tokens). For forgot-password / email-change / OTP / magic-link / email-confirmation flows, Phase 1f (Complete every flow) requires: receive email → click link → destination renders (NOT a 500) → fill + submit → verify side effect (session, password, email verified) → sign in new creds → confirm old creds rejected → confirm link single-use. See `memory/feedback_email_received_not_pass_on_recovery.md` + the Flow Completion Rule in CLAUDE.md.
- **Declaring standards compliance without a machine check fails repeatedly.** After refactoring 3 scaffolded specs to use POM fixtures, I told the user "all 3 specs follow CODE_STANDARDS.md" before running the audit. A 30-second grep found 14 POM violations in my own files (skipped tests, edge-case locators, failure-terminus assertions) plus 538 pre-existing across 21 other specs. Before reporting any spec/POM/fixture as "follows standards," run the 6-grep audit explicitly. Memory: `feedback_run_standards_audit_before_claiming_compliance.md`. Also enforced by `.github/workflows/standards-gate.yml` on PRs, but catching locally is faster than a red CI run.
- **Partner attribution gap in retest matrix.** Apr 14 retest marked Venn/Funnel/Renew theme-attribution ✅ based on theme visual only, no DB check. Retrospective retest (Apr 18) found `venn73458test` shortcode actually mis-attributes to partner **Simpson** at both `MoveInSessionTracker.moveInPartnerID` and `Referrals.referredBy` — identical pattern to ENG-2694 (Moved D2C → Simpson). Always DB-verify attribution for partner-shortcode flows, don't trust theme rendering as evidence. Phase 1d2 (Partner attribution verification) exists for exactly this reason.
