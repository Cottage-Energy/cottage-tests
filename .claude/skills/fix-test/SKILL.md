...
### Session: 2026-04-18 (ENG-2721 password-reset diagnosis — success-path vs error-path isolation)
- **Test BOTH success and error paths when diagnosing an endpoint 500.** When `/auth/confirm?token_hash=pkce_<valid>&type=recovery` returned a raw JSON 500, the instinct was "the endpoint is broken." Hitting the same endpoint with an invalid token (`pkce_invalid000…`) cleanly redirected to `/auth-code-error`. That proved the 500 was NOT a blanket endpoint failure — only the success code path was broken. Looking at the handler source confirmed: error branch returns `Response.redirect(auth-code-error)` and works; success branch runs `verifyOtp()` → `Response.redirect(next)` and 500s because the Supabase SSR cookie adapter can't persist the session on a plain `Response.redirect()` in TanStack. Narrowed the fix scope from "rewrite the route" to "fix the session-persist step in the success branch." See `memory/feedback_endpoint_success_vs_error_paths.md`.
- **When triaging an endpoint 500 before filing or fixing, fire at least three shapes of input**: valid (real bug), clearly-invalid (bad token / missing params / malformed payload), empty (param validation path). Report as "endpoint 500s on [specific branch]" with evidence the other branches work. More actionable for the dev and rules out "endpoint not deployed" / "middleware broke" / "runtime imploded."
- **TanStack framework-primitive mismatches are migration landmines.** Plain-web `Response.redirect()` can't forward `set-cookie` headers that the Supabase SSR helper writes. NextJS used `NextResponse.redirect()` + `createRouteHandlerClient({ cookies })` which wired the cookie adapter correctly. Handler code looks structurally identical between NextJS and TanStack; only an end-to-end flow test exposes the gap. When fixing a failing TanStack test, check if the failure is a framework-primitive mismatch, not a test-code problem — the test may be correct and the app is wrong. See `memory/feedback_tanstack_response_redirect_cookie_loss.md`.
